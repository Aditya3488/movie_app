      ...
      <div>
        <>
          <div className="main">
            <div className="row">
              <div className="col-3">
                <ul className="list-group favourite-genre">
                    {
                        this.state.genres.map((genre)=>(
                            this.state.currgen === genre ? 
                            <li className="list-group-item" style={{background:'#603CB5',color:'white',fontFamily:'bold'}}>{genre}</li> :
                            <li className="list-group-item" style={{backgroun:'white',color:'#603CB5',fontFamily:'bold'}} onClick={()=>this.handleGenreChange(genre)}>{genre}</li> 
                        ))
                    }
                </ul>
              </div>
                      ...
                      {filterarr.map((movieObj) => (
                        <tr>
                          <td> <img src={`https://image.tmdb.org/t/p/original${movieObj.backdrop_path}`}
                           alt={movieObj.title} 
                           style= {{width:'7rem'}} /> {movieObj.original_title} </td>
                          <td>{genreids[movieObj.genre_ids[0]]}</td>
                          <td>{movieObj.popularity}</td>
                          <td>{movieObj.vote_average}</td>
                          <td><button type="button" className="btn btn-danger" onClick= 
                         {()=>this.handleDelete(movieObj.id)}>Delete</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
